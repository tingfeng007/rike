import{E as e,m as t,t as n}from"./index-BbVwPC1P.js";function r(e){return e?e.trim().replace(/\/+$/,``):`https://api.deepseek.com`}function i(e){try{return JSON.parse(e)}catch{let t=e.match(/```(?:json)?\s*([\s\S]*?)\s*```/);if(t&&t[1])try{return JSON.parse(t[1])}catch{}let n=e.indexOf(`{`),r=e.lastIndexOf(`}`);if(n!==-1&&r!==-1&&r>n)try{return JSON.parse(e.slice(n,r+1))}catch{}let i=e.indexOf(`[`),a=e.lastIndexOf(`]`);if(i!==-1&&a!==-1&&a>i)try{return JSON.parse(e.slice(i,a+1))}catch{}throw Error(`未能从AI回复中解析出有效的JSON数据`)}}async function a({messages:e,temperature:n=.7,responseFormatJson:i=!1,onChunk:a}){let o=t.getSettings(),s=o.apiKey?.trim();if(!s)throw Error(`未配置 API Key，请先进入“设置”页面填入您的 API Key`);let c=`${r(o.baseUrl)}/chat/completions`,l={model:o.model||`deepseek-chat`,messages:e,temperature:n,stream:!0};i&&(l.response_format={type:`json_object`});let u=await fetch(c,{method:`POST`,headers:{"Content-Type":`application/json`,Authorization:`Bearer ${s}`},body:JSON.stringify(l)});if(!u.ok){let e=await u.text(),t=e;try{let n=JSON.parse(e);t=n.error?.message||n.message||e}catch{}throw Error(`AI请求失败 (${u.status}): ${t}`)}if(!u.body)throw Error(`当前环境不支持流式响应`);let d=u.body.getReader(),f=new TextDecoder(`utf-8`),p=``,m=``;for(;;){let{done:e,value:t}=await d.read();if(e)break;m+=f.decode(t,{stream:!0});let n=m.split(`
