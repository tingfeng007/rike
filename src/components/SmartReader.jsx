    ...
    <div className="flex flex-col h-full bg-slate-50 relative">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center bg-slate-100 rounded-lg p-0.5 text-xs text-slate-600">
              <button
                ...
                className={`px-2 py-0.5 rounded ${fontSize === 'text-sm' ? 'bg-white text-sky-600 shadow-xs font-semibold' : ''}`}
              >
                小
              </button>
              <button
                onClick={() => setFontSize('text-base')}
                className={`px-2 py-0.5 rounded ${fontSize === 'text-base' ? 'bg-white text-sky-600 shadow-xs font-semibold' : ''}`}
              >
                中
              </button>
              <button
                onClick={() => setFontSize('text-lg')}
                className={`px-2 py-0.5 rounded ${fontSize === 'text-lg' ? 'bg-white text-sky-600 shadow-xs font-semibold' : ''}`}
              >
                大
              </button>
            </div>
              ...
              >
                ...
                {articles.length > 1 && (
                  <button
                    ...
                    className="text-slate-500 hover:text-rose-500 p-0.5 rounded"
                  >
      ...
      {selectedWord && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-150">
          <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl p-5 shadow-2xl border border-slate-100 max-h-[85vh] overflow-y-auto">
            {/* Header */}
